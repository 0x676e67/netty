...
impl Opts {
    ...
    fn bench(self, b: &mut test::Bencher) {
        ...
        let url: http::Uri = format!("http://{}/hello", addr).parse().unwrap();
        ...
        let mut send_request = |req| {
            let fut = match client {
                Client::Http1(ref mut tx) => {
                    futures_util::future::Either::Left(tx.try_send_request(req))
                }
                Client::Http2(ref mut tx) => {
                    futures_util::future::Either::Right(tx.try_send_request(req))
                }
            };
            async {
