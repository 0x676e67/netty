//! HTTP/3 adapter comparison using the shared CLIENT_BENCH_* runtime matrix.
//! The normal benchmark rejects diagnostic H3_BENCH_* modes.
...
async fn measure(case: Case) -> Measurement {
    ...
    let server_endpoint = server_runtime
        .spawn(async move { endpoint(Some(server_config)) })
        .await
        .unwrap();
    let socket = std::net::UdpSocket::bind("127.0.0.1:0").unwrap();
    socket2::SockRef::from(&socket)
    ...
    let accept =
        server_runtime.spawn(async move { accepting.accept().await.unwrap().await.unwrap() });
    let (client, server) = tokio::join!(connecting, accept);
...
fn main() {
    assert!(std::env::var("H3_BENCH_PROFILE").is_err());
    assert!(std::env::var("H3_BENCH_DIRECT_TASKS").is_err());
    assert!(std::env::var("H3_BENCH_DIRECT_CLONE").is_err());
    runtime_matrix::run("http3", measure);
}
